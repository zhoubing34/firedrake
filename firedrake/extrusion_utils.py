...
def make_extruded_coords(extruded_topology, base_coords, ext_coords,
    ...
    if kernel is not None:
        ...
        kernel = op2.Kernel("""
void uniform_extrusion_kernel(double *base_coords, double *ext_coords, double *layer_height, int layer) {{
  for (int d = 0; d < {arity}; d++) {{
    for (int c = 0; c < {dim}; c++) {{
      ext_coords[2*{dim1}*d+c] = base_coords[{dim}*d+c];
      ext_coords[2*{dim1}*d+{dim1}+c] = base_coords[{dim}*d+c];
    }}
    ext_coords[2*d*{dim1}+{dim}] = *layer_height * (layer);
    ext_coords[2*d*{dim1}+{dim1}+{dim}] = *layer_height * (layer + 1);
  }}
}}""".format(arity=base_coords.cell_node_map().arity,
             dim=base_coords.function_space().value_size,
             dim1=base_coords.function_space().value_size + 1),  # dim1 is dimension of extruded mesh
                            "uniform_extrusion_kernel")
    elif extrusion_type == 'radial':
        kernel = op2.Kernel("""
void radial_extrusion_kernel(double *base_coords, double *ext_coords, double *layer_height, int layer) {{
  for (int d = 0; d < {arity}; d++) {{
    double norm = 0.0;
    for (int c = 0; c < {dim}; c++) {{
      norm += base_coords[d*{dim}+c] * base_coords[d*{dim}+c];
    }}
    norm = sqrt(norm);
    for (int c = 0; c < {dim}; c++) {{
      ext_coords[2*{dim}*d+c] = base_coords[d*{dim}+c] * (1 + (*layer_height * layer)/norm);
      ext_coords[2*{dim}*d+{dim}+c] = base_coords[d*{dim}+c] * (1 + (*layer_height * (layer+1))/norm);
    }}
  }}
}}""".format(arity=base_coords.cell_node_map().arity,
             dim=base_coords.function_space().value_size),
        ...
        kernel = op2.Kernel("""
void radial_hedgehog_extrusion_kernel(double *base_coords, double *ext_coords, double *layer_height, int layer) {{
  double v0[{dim}];
  double v1[{dim}];
  double n[{dim}];
  double x[{dim}] = {{0}};
   ...
   * (1  0) (y2 - y1)
   */
  n[0] = -(base_coords[{dim}+1] - base_coords[1]);
  n[1] = base_coords[{dim}] - base_coords[0];
  }} else if ({dim} == 3) {{
    /*
     * normal is
     * v0 x v1
     ...
     */
    for (i = 0; i < 3; ++i) {{
      v0[i] = base_coords[{dim}+i] - base_coords[i];
      v1[i] = base_coords[2*{dim}+i] - base_coords[i];
    }}
  ...
  for (d = 0; d < {arity}; ++d) {{
    for (c = 0; c < {dim}; ++c ) {{
      ext_coords[2*d*{dim}+c] = base_coords[d*{dim}+c] + n[c] * layer_height[0] * layer / norm;
      ext_coords[(2*d+1)*{dim}+c] = base_coords[d*{dim}+c] + n[c] * layer_height[0] * (layer + 1)/ norm;
    }}
  }}
}}""".format(arity=base_coords.cell_node_map().arity,
            dim=base_coords.function_space().value_size),
